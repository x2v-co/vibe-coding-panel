...
Only transcript text is sent to the computer's configured correction provider;
audio, project files and task history are not sent for correction. Correction
follows the currently bound target: Codex uses the installed Codex CLI and its
resolved provider, while Claude uses the installed Claude Code CLI or its
configured Messages-compatible service. This can consume the configured
provider's usage allowance.

`PANEL_CORRECTION_MODEL` overrides the correction model where the selected CLI
supports it. Providers without a compatible configuration, model or login safely
retain the original transcript. The correction path never receives project files,
tools, MCP servers, hooks or session history.
