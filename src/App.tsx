  ...
  return (
    <div className={`app-shell layout-${layout} ${isPanelMode ? 'panel-mode' : ''}`} data-theme={theme}>
      <header className="topbar">
        <a className="brand" href="/" title="返回项目首页" aria-label="返回项目首页"><span className="brand-mark"><Sparkles size={17} /></span><span>VIBE PANEL</span><small>AGENT WORKSPACE</small></a>
        <div className="topbar-actions"><button className="connection" onClick={() => setShowSettings(true)} title={`${agentLabel} / Agent 连接设置`}><i className={connectionState} /><span>{connectionState === 'checking' ? 'CONNECTING' : connection.mode === 'demo' ? agentLabel : activeAgent.label.toUpperCase()}</span></button><a className="header-button" href="/api/desktop-controller/view" title="打开手机遥控器">遥控器</a><button className="header-button" onClick={() => setShowHistory(true)}><History size={18} /><span>历史</span></button><button className="header-button primary" onClick={reset}><Plus size={18} /><span>新任务</span></button></div>
      </header>

      <main>
        <section className="intro"><div className="intro-index">VP / 01</div><div><p className="eyebrow">AGENT WORKSPACE</p><h1>把 Agent，<br />做成工作流。</h1></div><p className="intro-copy"><strong>电脑端工作台。</strong>连接本机 Agent，选择 Workspace，创建任务、查看过程并继续原生会话。手机输入由配套的 Vibe Remote 提供。</p></section>

        <section className="console-zone" aria-label="Agent 工作台">
...
const siteCopy = {
  zh: {
    ...
    threeSteps: 'THREE STEPS', panelHow: '从电脑开始，|让 Agent 进入项目。', startConnector: '启动 Connector', startConnectorDescription: '解压后启动 Connector，它会检查 Codex、Claude Code 和语音组件。', connectAgent: '连接 Agent 与 Workspace', connectAgentDescription: '选择本机 Agent 和项目目录，Panel 会保留任务与原生会话入口。', startWork: '开始执行', startWorkDescription: '直接输入、录音或捕获上下文，然后在电脑上确认和查看结果。',
    remoteKicker: 'VIBE REMOTE / COMPANION', remoteQuestion: '需要离开电脑输入？', remoteDescription: 'Vibe Remote 是 Vibe Panel 的手机配套入口，用语音和按键追加当前电脑会话。', learnRemote: '了解 Vibe Remote',
    say: '说', sayDescription: '录音后校对，追加到电脑草稿', press: '按', pressDescription: '用快捷键和输入模板控制操作', send: '发', sendDescription: '在手机确认后提交电脑当前会话',
    remoteHero: '手机负责输入，|电脑继续工作。', remoteDescriptionLong: 'Vibe Remote 是 Vibe Panel 的配套遥控器。用手机语音和按键追加当前电脑会话，完整草稿与回复留在电脑。', openRemote: '打开手机遥控器', setup: '查看启动方式', noAccount: '无需注册；由电脑 Connector 生成一次性配对入口',
    remoteHow: '一次连接，|随时继续输入。', startControl: '在电脑启动控制中心', startControlDescription: '使用 Connector 的控制中心模式，绑定 Codex App、Claude App 或 Claude Code 会话。', scanQr: '扫描二维码配对手机', scanQrDescription: '电脑会生成带配对码的一次性入口。手机首次连接后，之后可直接打开遥控器。', remoteAppend: '手机追加，电脑执行', remoteAppendDescription: '语音和按键只写入电脑草稿；确认发送后，Agent 继续在电脑项目中工作。', partOfPanel: 'PART OF VIBE PANEL', noComputer: '还没有电脑端连接？', noComputerDescription: '先安装 Connector 并打开 Vibe Panel，再从控制中心启用手机遥控。', learnPanel: '了解 Vibe Panel',
  ...
  en: {
    ...
    threeSteps: 'THREE STEPS', panelHow: 'Start on your computer,|then bring your Agent into the project.', startConnector: 'Start Connector', startConnectorDescription: 'Unpack and launch Connector. It checks Codex, Claude Code, and speech components.', connectAgent: 'Connect Agent and Workspace', connectAgentDescription: 'Choose a local Agent and project folder. Panel keeps task history and native session entry points together.', startWork: 'Start working', startWorkDescription: 'Type, record, or capture context, then confirm and review the result on your computer.',
    remoteKicker: 'VIBE REMOTE / COMPANION', remoteQuestion: 'Need to type away from your computer?', remoteDescription: 'Vibe Remote is the phone companion for Vibe Panel. Use voice and keys to add to the current computer session.', learnRemote: 'Learn about Vibe Remote',
    say: 'Say', sayDescription: 'Proofread a recording and add it to the computer draft', press: 'Press', pressDescription: 'Use shortcuts and input templates to control actions', send: 'Send', sendDescription: 'Confirm on your phone and submit the current computer session',
    remoteHero: 'Your phone handles input,|your computer keeps working.', remoteDescriptionLong: 'Vibe Remote is the companion controller for Vibe Panel. Use phone voice and keys to add to the current computer session while the full draft and response stay on the computer.', openRemote: 'Open phone remote', setup: 'See how to start', noAccount: 'No account required; Connector creates a one-time pairing entry on your computer',
    remoteHow: 'Connect once,|keep sending input anywhere.', startControl: 'Start Control Center on your computer', startControlDescription: 'Use Connector Control Center to bind a Codex App, Claude App, or Claude Code session.', scanQr: 'Scan the QR code', scanQrDescription: 'Your computer shows a one-time entry with a pairing code. After the first connection, open Remote directly from your phone.', remoteAppend: 'Add from your phone, run on your computer', remoteAppendDescription: 'Voice and key input only update the computer draft. After you confirm, the Agent continues in the project on your computer.', partOfPanel: 'PART OF VIBE PANEL', noComputer: 'No computer connection yet?', noComputerDescription: 'Install Connector and open Vibe Panel first, then enable phone remote from Control Center.', learnPanel: 'Learn about Vibe Panel',
    oneConnector: 'ONE CONNECTOR / TWO ENTRIES', downloadTitle: 'Download Connector.', downloadDescription: 'One Connector supports both Vibe Panel and Vibe Remote. Your Agent, projects, and pairing service run on your computer.', beta: 'Preview release · Includes Node.js 24 and runtime dependencies · Unpack and launch', releaseChecks: 'Releases and SHA256 checksums', twoEntries: 'Two entries, one install', panelWorkspace: 'Vibe Panel · Computer workspace', remoteCompanion: 'Vibe Remote · Phone companion', launchControl: 'Enable Control Center in Connector', launchControlDescription: 'Run Control Center and bind the current computer session. Your computer will show a one-time pairing QR code.', scanPhone: 'Scan with your phone', scanPhoneDescription: 'After pairing, open Remote directly. Voice and keys add to the computer draft while the full process and response stay in Panel.', packageNote: 'These portable packages are not signed or notarized and are not system installers. macOS may ask you to allow them in Privacy & Security. Linux requires a Node 24 compatible glibc system (Ubuntu tested).', fullInstall: 'Full installation guide', installForAgent: 'Install with an Agent', noAgent: 'No Agent installed yet?', demo: 'Try demo mode first', projectIntro: 'About', openRemoteNav: 'Open Remote', privacy: 'Privacy', terms: 'Terms', moreProducts: 'More products', language: '中文', languageLabel: '切换到中文',
  },
} as const;

function useSiteLanguage(): [SiteLanguage, () => void] {
  const [language, setLanguage] = useState<SiteLanguage>(() => {
    const query = new URLSearchParams(window.location.search).get('lang');
    if (query === 'en' || query === 'zh') return query;
    const stored = localStorage.getItem('vibe-site-language');
    if (stored === 'en' || stored === 'zh') return stored;
    return navigator.language.toLowerCase().startsWith('zh') ? 'zh' : 'en';
  });
  useEffect(() => {
    localStorage.setItem('vibe-site-language', language);
    document.documentElement.lang = language === 'zh' ? 'zh-CN' : 'en';
  }, [language]);
  return [language, () => setLanguage((current) => current === 'zh' ? 'en' : 'zh')];
}

function SiteHeader({ remote = false, language, toggleLanguage }: { remote?: boolean; language: SiteLanguage; toggleLanguage: () => void }) {
  const copy = siteCopy[language];
  return <header className="site-nav"><a className="site-brand" href={remote ? '/remote' : '/'}><span className="site-mark">{remote ? <Smartphone size={16} /> : <Sparkles size={16} />}</span><span>{remote ? 'VIBE REMOTE' : 'VIBE PANEL'}</span></a><nav><a href={remote ? '/' : '#how-it-works'}>{remote ? 'Vibe Panel' : copy.panelNav}</a><a href={remote ? '#how-it-works' : '/remote'}>{remote ? copy.panelNav : copy.remote}</a><a href={remote ? '/download#controller-setup' : '/download'}>{remote ? copy.startConnector : copy.download}</a><a className="site-nav-action" href={remote ? '/api/desktop-controller/view' : '/app'}>{remote ? copy.openRemoteNav : copy.openPanel} <ChevronRight size={15} /></a><button type="button" className="site-language" onClick={toggleLanguage} aria-label={copy.languageLabel}>{copy.language}</button></nav></header>;
}
...
function RemoteLandingPage() {
  ...
  </main><SiteFooter language={language} remote /></div>;
}
...
function App() {
  ...
  if (url.pathname === '/remote') return <RemoteLandingPage />;
