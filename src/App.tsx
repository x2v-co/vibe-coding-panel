          ...
          <div className="device">
            ...
            <div className="display-bezel"><div className="display">
              ...
              <div className="display-status"><span className={`status-light ${status}`} /><span>{activeAgent.label.toUpperCase()} / {nativeSelection ? `SESSION · ${nativeSelection.provider === 'claude' ? 'CLAUDE' : 'CODEX'}` : isRecoveringJob ? '恢复中' : statusLabels[status]}</span><div className="display-quick-actions"><button type="button" className="new-task-action" onClick={reset} disabled={busy || voiceBusy || isAddingContext || isRecoveringJob} title="新建任务" aria-label="新建任务"><Plus size={16} /></button><button type="button" className="context-action" onClick={() => imageInputRef.current?.click()} disabled={busy || voiceBusy || isAddingContext || isRecoveringJob} title="添加图片上下文" aria-label="添加图片上下文">{isAddingContext ? <RotateCw className="spin" size={16} /> : <ImagePlus size={16} />}</button><button type="button" className="history-action" onClick={() => setShowHistory(true)} title="任务记录" aria-label="任务记录"><History size={16} /></button>{layout === 'hardware-micro' && <button type="button" className="micro-toolbar-action" onClick={() => setShowSettings(open => !open)} title="设置主题与工作目录" aria-label="设置主题与工作目录"><Palette size={16} /></button>}</div><span className="display-time"><Clock3 size={13} /> {busy ? duration : 'READY'}</span></div>
                ...
                {layout === 'hardware-micro' && result && !busy && !capturePreview && <textarea id="command" className="micro-followup" ref={promptRef} aria-label="继续当前任务" rows={2} value={prompt} maxLength={3000} placeholder={isListening ? '正在录音…' : isTranscribing ? '正在转写…' : '继续当前任务…'} onChange={(event) => setPrompt(event.target.value)} />}
